...
5. **Parallelism** — `/spec` auto-generates Parallel Batches from task `files:` and `depends:` metadata. Shared files are classified as exclusive, shared-additive (accumulator pattern), or shared-mutative (must serialize). Inter-spec parallelism works today via separate worktrees.
